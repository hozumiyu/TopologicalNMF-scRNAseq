...
if not exists(temp_folder + 'GSE75748_sc_cell_type_ec.csv.gz'):   #download raw data
    wget.download('https://ftp.ncbi.nlm.nih.gov/geo/series/GSE75nnn/GSE75748/suppl/GSE75748_sc_cell_type_ec.csv.gz', out=temp_folder)   #download data
else:
    print('RAW data already in temporary folder')

    
with gzip.open(temp_folder + 'GSE75748_sc_cell_type_ec.csv.gz') as f:
...
aux = pd.read_csv('%s/code/auxilary_file/'%(data_process_path) + 'GSE75748_sample.csv')
